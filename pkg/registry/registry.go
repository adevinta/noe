...
func (r *PlainRegistry) listArchsWithAuth(ctx context.Context, client http.Client, auth AuthenticationToken, registry, image, tag string) ([]Platform, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("failed to get manifest list for image %s in registry %s. Unexpected status code %d. Expecting %d", image, registry, resp.StatusCode, http.StatusOK)
	}
